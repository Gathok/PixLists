package de.malteans.pixlists.list.data.repository

import de.malteans.pixlists.list.data.database.PixDao
...
import de.malteans.pixlists.list.data.mappers.toPixCategory
import de.malteans.pixlists.list.data.mappers.toPixColor
import de.malteans.pixlists.list.data.mappers.toPixList
import de.malteans.pixlists.list.domain.PixCategory
import de.malteans.pixlists.list.domain.PixColor
...
class DefaultPixRepository(
    ...
    override suspend fun getAllLists(): Flow<List<PixList>> {
        return dao.getAllLists().map { list ->
            list.map { it.toPixList() }
        }
    }

    override suspend fun getFullListById(listId: Long): PixList {
        val categories = dao.getCategoriesForList(listId).first().associate { it.id to it.toPixCategory() }
        val entries = dao.getEntriesForList(listId).first().associate { PixDate.fromString(it.date) to categories[it.categoryId]!! }
        return dao.getList(listId).first().toPixList(categories.values.toList(), entries)
    }
    ...
    override suspend fun getCategoriesForList(listId: Long): Flow<List<PixCategory>> {
    ...
    override suspend fun getAllColors(): Flow<List<PixColor>> {
